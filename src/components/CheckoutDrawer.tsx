...
import {
  ...
  BookOpen, ChevronLeft, Tag, Calculator,
} from "lucide-react";
...
const PAYMENT_METHODS = [
  { method: "PIX" as PaymentMethod, label: "PIX", icon: QrCode, color: "#00805A", glow: "#00805A30" },
...
export function CheckoutDrawer({
  isOpen, onOpenChange, orderId, totalAmount, clients, onSuccessReset,
}: CheckoutDrawerProps) {
  const router = useRouter();
  const [step, setStep] = useState<"METHOD" | "CASH_CHANGE" | "TAB_CLIENT" | "SUCCESS">("METHOD");
  ...
  const handleMethodClick = (method: PaymentMethod, isCash?: boolean, isTab?: boolean) => {
    if (isCash) {
      ...
      >
        ...
        {step === "SUCCESS" && (
          <div className="flex flex-col items-center justify-center py-16 gap-4">
            ...
            <p className="text-3xl font-black" style={{ color: "#00805A" }}>
              {BRL(finalTotal)}
            </p>
          </div>
        )}
          ...
          <div className="px-6 py-6 space-y-5 overflow-y-auto">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                {PAYMENT_METHODS.map(({ method, label, icon: Icon, color, glow, isCash, isTab }) => (
                  <button
                    key={method}
                    type="button"
                    disabled={isProcessing}
                    onClick={() => handleMethodClick(method, isCash, isTab)}
